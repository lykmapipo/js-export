...
function prepareMultiSheets() {
    ...
    //prepare sheets
    function prepareSheets(sheetName, data, key) {

        _.forEach(data, function(value, _key) {
            //prepare sheet name
            var name = _key && _.isString(_key) ? _key : sheetName;
            ...
            if (isValidSheet) {
                ...
                //prepare sheet rows
                _.forEach(self.data, function(data) {
                    var _data = {};

                    _.forEach(sheet.columns, function(column) {
                        _data[column.name] =
                            _.get(data, column.ref, self.options.missing);
                    });
            ...
            _.forEach(value, function(subvalue, subkey) {
                if (_.isPlainObject(subvalue)) {
                    ...
                    if (_.isString(_key)) {
                        value.ref = [_key, subkey].join('.');
                    }

                    subvalues[subkey] = subvalue;
                }
            });
            prepareSheets(name, subvalues, _key);
